...
const STATUS_ORDER = ['CONFIRMED', 'PREPARING', 'READY', 'PICKED_UP']
...
export default function OrderSuccessPage({ successData, onNavigate }) {
  ...
  // Live status updates from canteen dashboard actions
  useEffect(() => {
    const channel = supabase
      .channel(`success-order-${order.id}`)
      .on(
        'postgres_changes',
        {
          event:  'UPDATE',
        ...
        ({ new: updated }) => {
          if (STATUS_CONFIG[updated.status]) setStatus(updated.status)
        }
      )
      .subscribe()
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-10">
      {/* Hero */}
      <div
        className={`${status === 'READY' ? 'gradient-success' : 'gradient-hero'} text-white p-8 text-center transition-all duration-700`}
      >
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 rounded-xl transition-all ${
                    isCurr ? `${c.bg} ${c.border} border-2` : isDone ? 'opacity-50' : 'opacity-30'
                  }`}
                >
                  <span className="text-2xl">{c.icon}</span>
                  <div>
                    <p className={`font-semibold text-sm ${isCurr ? c.color : 'text-gray-600'}`}>{c.label}</p>
                    {isCurr && <p className="text-xs text-gray-400">Current status</p>}
                  </div>
                  {isCurr && <div className="ml-auto w-3 h-3 bg-primary rounded-full animate-pulse" />}
                </div>
              )
            })}
          </div>
