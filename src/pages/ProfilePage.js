      ...
      <div className="max-w-lg mx-auto px-4 mt-4 space-y-4">
        ...
        <div className="bg-white rounded-2xl p-5 border border-orange-100 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">student@campus.edu</p>
        ...
        <div className="bg-white rounded-2xl overflow-hidden border border-orange-100">
          {[
            { icon: <ShoppingBag className="w-5 h-5 text-primary" />, label: 'My Orders', sub: 'View order history', action: 'orders' },
            { icon: <ChefHat className="w-5 h-5 text-gray-600" />, label: 'Canteen Dashboard', sub: 'Staff only — manage orders', action: 'canteen' },
          ].map((item, i) => (
            <button key={i} onClick={() => onNavigate(item.action)}
              className={`w-full flex items-center gap-4 p-4 text-left hover:bg-gray-50 transition-colors ${i > 0 ? 'border-t border-gray-100' : ''}`}>
              <div className="w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center">{item.icon}</div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-white/80 text-sm leading-relaxed">
                CampusEats uses direct Merchant UPI (P2M). Unlike Swiggy/Zomato charging 15–25% commission,
                your money goes directly to the canteen — ₹0 extra per order.
              </p>
        ...
        <div className="bg-white rounded-2xl p-4 border border-orange-100 text-center">
          ...
          <p className="text-gray-400 text-xs mt-1">v1.0.0 • Built for Indian Colleges</p>
          <p className="text-gray-400 text-xs">Powered by Supabase + Direct UPI</p>
        </div>
      </div>
    </div>
  )
